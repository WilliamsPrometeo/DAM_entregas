 ...
 * </p>
 *
 * @author Alumno- Adam "Patata" Pérez
 * @version 4.0
 */
public class Utilidades {

    private static final recursos.MyScanner sc = new recursos.MyScanner();
    ...
    public static void copiar_archivo(String origen, String destino) {
        ...
        if (directorio.isDirectory() && directorio.exists()) {
            ...
            if (archivos != null) {
                for (File file : archivos) {
                    if (file.isFile()) {
                        System.out.printf("- %3s: %5d bytes %10s %n", file.getName(), file.length(), obtenerFechaModificacion(ruta+file.getName()));
                    }
                }
            } else {
                System.out.println("El directorio vacio");
            }
        } else  {
    ...
    public static void listarDirectorios(String ruta) {
        ...
        if (directorio.exists()&&directorio.isDirectory())  {
            ...
            if (archivos != null&&archivos.length>0) {
                ...
                for (File file : archivos) {
                    if (file.isFile()) {
                        System.out.printf("- %20s | %5d bytes | Fecha de modificación:  %10s %n ", file.getName(), file.length(), obtenerFechaModificacion(ruta+file.getName()));
                    }
                }
                for (File file : archivos) {
                    if (file.isDirectory()) {
