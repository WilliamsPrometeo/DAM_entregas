...
public class Utilidades {
    ...
    public static void listarDirectories(String ruta) {
        ...
        if (directorio.exists() && directorio.isDirectory()) {
            ...
            if (archivos != null && archivos.length > 0) {
                for (File file : archivos) {
                    if (file.isFile()) {
                        System.out.printf("- %20s | %5d bytes | %s%n", file.getName(), file.length(), obtenerFechaModificacion(ruta + file.getName()));
                    }
                }
                for (File file : archivos) {
                    if (file.isDirectory()) {
                        listarDirectories(directorio + "/" + file.getName() + "/");
                    }
                }
            } else {
