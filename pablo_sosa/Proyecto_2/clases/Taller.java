...
public class Taller {
    ...
    public void registrarVehiculo() {           /*Metodo de registrarVehiculo*/
        ...
        String matricula;
        enums.TipoVehiculo tipo = null; //Tipo nulo para iniciar
        do {
    ...
    public void filtrarTipoVehiculo(){      //Inicializo el filtrarTipoVehiculo para luego poder filtrar por TipoVehiculo
        ...
        for (Vehiculo vehiculo : vehiculos) {
            if (TipoVehiculo == vehiculo.getTipoVehiculo()) {
                System.out.println(vehiculo);       //Te muestra los vehiculos que cumplen los requisitos del filtro
            }
        }
    }

    public Servicio buscarServicio(String descripcion) {        //Inicializo el método de buscarServicio
        for (Servicio servicio : catalogoServicios) {           // Mediante el catálogo
            if (servicio.getDescripcion().equals(descripcion)) {        //Comparo con equals()
                return servicio;
            }
        }
        return null;
    ...
    public void mostrarVehiculos() {        //Inicializo mostrarVehiculos para que me de una lista con estos
        ...
        if (!vehiculos.isEmpty()) {
            ...
            do {
                char opcion = sc.pedirLetra("¿Quieres filtrar por tipo? s/n");      //filtro
                switch (opcion) {
                    case 's':
                    case 'S':
                    correcto = true;
    ...
    public Vehiculo buscarVehiculo(String matricula) throws VehiculoNoEncontrado {      //Inicializo buscarVehiculo que busca los vehiculos asignados
        for (Vehiculo vehiculo : vehiculos) {
            if (vehiculo.getMatricula().equals(matricula)) {        //Comparo matriculas con equals para saber si esta o no
                return vehiculo;
            }
        }
        return null;
